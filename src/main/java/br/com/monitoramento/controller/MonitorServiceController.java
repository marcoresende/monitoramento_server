...
//        }
		...
		String message = !(config == null) ? " resultado encontrado." : "Pesquisa n„o retornou nenhum resultado!";
    ...
    public ResponseEntity<BaseResponse> config(@RequestBody ConfigRequest request) throws FalhaException {
        if(!isValidRequest(request)){
            BaseResponse response = new BaseResponse(HttpStatus.BAD_REQUEST.value(), "Campos obrigatÛrios!");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
        }
        ...
        BaseResponse response = new BaseResponse(HttpStatus.OK.value(), "Configura√ß√µes alteradas com sucesso!");
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
